...
def catch_ball(angle_degrees, signed_angle_degrees, robotInterface) -> np.ndarray:
    if (angle_degrees is not None
        and signed_angle_degrees is not None
        and angle_degrees < 1):
        robotInterface.send_command("move", 300, 10)
    else:
        robotInterface.send_command("turn", signed_angle_degrees * -1 / 3, 10)
...
def run_video(robotInterface: RobotInterface.RobotInterface):
    ...
    while True:
        ...
        if (
            ...
            dist_to_ball = ball_vector[0] ** 2 + ball_vector[1] ** 2
            print(f"Ball vector: {ball_vector}")
            print(f"Ball vector length: {dist_to_ball}")
            if dist_to_ball < 140:
                print("Ball is close")
                is_catching_ball = True
                catch_ball(signed_angle_degrees, angle_degrees, robotInterface )
                return
            else:

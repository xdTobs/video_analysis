...
class Analyse:
    ...
    def apply_theshold(
        self, image: np.ndarray, bounds_dict_entry: np.ndarray, out_name
    ) -> np.ndarray:
        ...
        if "white-ball" == out_name:
            lower = np.array([0, 0, 200])
            upper = np.array([170, 55, 255])
        else:
    ...
    def isolate_borders(
        ...
        mask = self.apply_theshold(image, bounds_dict_entry, out_name=out_name)

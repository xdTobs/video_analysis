...
class Steering():
    def __init__(self, online = True, host = "", port = 0):
        ...
        self.update_interval = 15  # Time in seconds
    ...
    def find_ball_vector(self, keypoints: np.ndarray, robot_pos: np.ndarray, robot_vector: np.ndarray) -> np.ndarray:
        ...
        if self.target_position is not None:
            ...
            if self.time_to_switch_target < self.update_interval:
                if distance_to_target <= self.distance_threshold_max and distance_to_target >= self.distance_threshold_min:
    ...
    def pick_program(self, keypoints: np.ndarray, robot_pos: np.ndarray, robot_vector: np.ndarray, distance_to_closest_border: float):
        ...
        try:
            ...
            if dist_to_ball < self.distance_threshold_max:
